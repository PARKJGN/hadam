...
//sse라는 이름으로 evt가 발생하면 감지해서 어떤 알림이 왔는지 noti해주는 함수 -건일
	eventSource.addEventListener('sse', event => {
		...
		// 알림내용
		let aaaaaaaaaaaaaa = splittedData1[0];

		
		if(aaaaaaaaaaaaaa!=="EventStream Created"){
		$("#nonealram").remove();
		...
		// 알림내용
		let notifyComment = splittedData[0];
		// 게시물 번호
		...
		switch (notifyComment) {
		  case "새로운 댓글이 달렸습니다":
		    aTageHref ="/community/boardView?boardId="+secondPart;
		    break;
		  case "새로운 참가 신청이 들어왔습니다":
			aTageHref =`/community/boardView?${memberIndex}`;
		...
		const newLi = document.createElement("li");
				...
				// 이벤트 데이터를 <a> 요소의 텍스트 콘텐츠로 설정
				newLink.textContent = notifyComment;
				// <a> 요소의 href 속성을 설정 (예시로 'index.html'로 설정)
				newLink.href = aTageHref;
