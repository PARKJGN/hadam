		...
		<div class="list-main-wrap fl-wrap card-listing" id="box">
			...
			<c:forEach items="${map}" var="map" varStatus="outerIndex">
				...
				<div class="scheduleTitle" id="scheduleTitle">
					<h5>
						Schedule : <span><a href="">${map.key.boardTitle} </a></span>
					</h5>
				</div>
										...
										<div class="geodir-category-content-title-item">
											...
											<div class="geodir-category-location fl-wrap">
												<a href="#" class="map-item"><i
													class="fas fa-map-marker-alt"></i>${schedule.locationName }</a>
											</div>
										</div>
									</div>
			...
			<div class="pagination" id="page">
				<!-- 페이지 맨처음으로 이동 -->
				<a href="/community/scheduleShareList?page=1" class="prevposts-link"><i
					class="fa fa-caret-left"></i></a>
				<!-- 전페이지로 이동 -->
				<a href="/community/scheduleShareList?page=${prevMax}"
				...
				<!-- 다음페이지 이동 -->
				<a href="/community/scheduleShareList?page=${nextMin}"
					class="nextposts-link"><i class="fa fa-caret-right"></i></a>
				<!-- 마지마페이지 이동 -->
				<a href="/community/scheduleShareList?page=${lastPage}"
					class="nextposts-link"><i class="fa fa-caret-right"></i></a>
			</div>
	...
	<div class="modal-dialog modal-fullscreen ">
		...
		<div class="modal-content asdf2" style="height: 500px;">
			...
			<div class="modal-body">
				...
				<form class="custom-form" action="scheduleAttendWrite"
					id="contactform" method="post">

					<!-- boardId, scheduleTableId, memberIndex 값 가져오기 -->
					<c:forEach items="${map}" var="map" varStatus="outerIndex">
						<input type="hidden" name="boardId" value="${map.key.boardId}" />
						<input type="hidden" name="scheduleTableId"
							value="${map.key.scheduleTableId}" />
					</c:forEach>
